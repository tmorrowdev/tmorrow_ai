...
**Default dialect is Snowflake.** Use Snowflake syntax (DATEADD, DATEDIFF, ILIKE, VARIANT, FLATTEN, etc.) unless the user specifies otherwise.

When the user wants to run queries in SageMaker Studio notebooks, also provide a Python wrapper using `snowflake-connector-python`:
```python
import pandas as pd
query = """
