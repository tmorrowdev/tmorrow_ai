...
**If Snowflake MCP is connected:**

1. Resolve the table name (handle database.schema.table format, use `SHOW TABLES` or `INFORMATION_SCHEMA` to suggest matches if ambiguous)
2. Run `DESCRIBE TABLE` to get column names, types, and metadata
3. Run Snowflake profiling queries against the live data (use `SAMPLE` for large tables)
...
1. Ask the user to provide a Snowflake table name (database.schema.table) or upload a file
2. If they describe a table schema, provide Snowflake profiling queries they can run in SageMaker or Snowflake directly
