...
1. Explore the Snowflake schema to find relevant tables and columns
2. Write Snowflake SQL query(ies) to extract the needed data
3. Execute the query and retrieve results
4. If the query fails, debug and retry (check column names, fully-qualified table references, Snowflake syntax)
5. If results look unexpected, run sanity checks before proceeding

**If no Snowflake connection is available:**

1. Ask the user to provide data in one of these ways:
   - Paste query results directly
   - Upload a CSV or Excel file
   - Describe the schema so you can write Snowflake SQL for them to run in SageMaker or Snowflake directly
2. If writing queries for manual execution, use Snowflake syntax (see the `sql-queries` skill)
