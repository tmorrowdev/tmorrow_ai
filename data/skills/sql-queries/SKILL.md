---
name: sql-queries
description: Write correct, performant Snowflake SQL. Also supports BigQuery, Databricks, PostgreSQL, and other dialects. Use when writing queries, optimizing slow SQL, translating between dialects, or building complex analytical queries with CTEs, window functions, or aggregations.
---

# SQL Queries Skill

Write correct, performant, readable SQL. **Default dialect is Snowflake** — use Snowflake syntax unless the user specifies otherwise. When generating code for SageMaker notebooks, wrap queries in `snowflake-connector-python` calls.
